...
func (game *Game) play(bet float64) float64 {
	...
	if playerHAsBlackJack && !dealerHasBlackjack {
		...
		fmt.Println("The dealer other card is:", game.dealerCards[1].getString())
...
func (game *Game) dealerTurn() {
	...
	for true { 
		...
		if shouldDealerHit(game.dealerCards) {
			card := game.deck.deal(1)[0]
			game.dealerCards = append(game.dealreCards, card)
...
func enterString() string {
	...
	input, err := reader.ReadString('\r') // if \r doesn't work use \n
	if err != nil {
...
func main() {
	...
	for balance > 0
	fmt.Printf("Your balance is $%.2\n", balnce)
	fmt.Printf("Enter your bet (q to quiet): ")
